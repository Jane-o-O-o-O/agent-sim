...
```python
from agent_sim import LLMAgent, OpenAIBackend, TokenBucketRateLimiter

backend = OpenAIBackend(
    model="gpt-4o-mini",
    api_key="sk-...",
    base_url=None,  # 或自定义端点
)
...
```python
from agent_sim import EvalSuite, EvalReport
...
```python
from agent_sim import export_messages_to_json, export_messages_to_markdown

export_messages_to_json(bus.history, "output/chat.json")
export_messages_to_markdown(bus.history, "output/chat.md", title="对话记录")
```
